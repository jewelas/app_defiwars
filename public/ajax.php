...
if ($request == "login") {
  ...
  if ($nonce) {
    ...
    // Prepared statement to protect against SQL injections
    $stmt = $conn->prepare("INSERT INTO users (address, nonce, email) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $address, $nonce, $email);
...
if ($request == "contact") {
    $stmt = $conn->prepare("INSERT INTO contacts (name, website, email, message) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $data->name, $data->website, $data->email, $data->message);
